...
# KG与LLM结合的范式
![[TOG.png]]
1. LLM-only：大模型本身的能力或者经过ft、cot微调后的大模型的能力回答问题 -- 知识不充分无法回答
2. LLM+kg：使用大模型将question转化为知识图谱的查询语句获得相关三元组，再将三元组结合大模型回答问题 -- 知识图谱不够健全则无法回答
3. LLMxkg：LLM沿着知识图谱的推理路径逐步“思考”来获得最终答案 -- paper所阐述的观点

# 流程
![[TOG 1.png]]
1. 初始化（INITIALIZATION）：使用prompt模版输入LLM对question提取topic entities，保留前N个topic entities，这里也可以少于N
2. 探索（EXPLORATION）：
    - 关系探索（Relation Exploration）：
        - 搜索（Search）：对N个entities（如果是第一次开始关系探索则是初始化的N个topic entities）使用你所使用的知识图谱的查询语言获得所有的候选关系集合$R_{cand}$$和$$P_{cand}$路径（如上图的Reasoning paths）
        - 剪枝（Prune）：利用LLM根据question和候选集$R_{cand}$从$P_{cand}$中选择前N条推理路径P，prompt如下：![[TOG 2.png]]
	- 实体探索（Entity Exploration）：
		- 搜索（Search）：从上面获得的N个关系集（路径P取的前N个，自然有N个关系被保留）去获得tail entities，使用你所使用的知识图谱的查询语言获得所有的候选实体集合$E_{cand}$和新的$P_{cand}$路径
		- 剪枝（Prune）：利用LLM根据question和候选实体集$E_{cand}$$从$$P_{cand}$中选择前N条推理路径P，prompt如下：![[TOG 3.png]]
3. 推理（REASONING）：使用prompt的模版对已有的推理路径P做推断，看能否回答问题，如果能则使用推理路径和查询作为输入到LLM生成答案，如果不能则重复2探索和3的推理，直到能回答问题或达到最大搜索深度$D_{max}$$，如果达到最大深度$$D_{max}$，仍然无法通过N条推理路径P解决问题，这种情况下只根据LLM的内在知识回答问题
	- 推理prompt：![[TOG 4.png]]
	- 生成prompt：![[TOG 5.png]]

其它：
1. 对于prune剪枝部分，使用的都是LLM去评分会增加LLM的调用次数导致响应时间增长，可以使用轻量级模型如BM25和SentenceBERT也可以作为剪枝工具在Exploration阶段使用，根据它们与question的字面相似度来选择前N个实体和关系，降低性能，提升效率，即使如此也比纯使用LLM的性能好，如果换了轻量模型做剪枝，我们可以增大N的宽度来提高性能
...
# 代码思路去看
code/TOG_code\.zip/ToG/main_freebase.py，写了大量注释，能解决一些论文上的疑惑
...
15. {wiki.relation.father (Score: 0.4)}: Uncle is father's brother, so father might provide some information as well.
16. {wiki.relation.position held (Score: 0.1)}: This relation is moderately relevant as it can provide information about any significant positions held by Mesih Pasha or his uncle that could be related to becoming an emperor.
...
10. {wiki.relation.donations (Score: 0.3)}: This relation is relevant because it can provide information about the financial contributions made to the Van Andel Institute, which may include donations from the American businessman in question.
11. {wiki.relation.educated_at (Score: 0.3)}: This relation is relevant because it can provide information about the educational background of the American businessman, which may have influenced his involvement in founding the Van Andel Institute.
...
利用知识图谱引导检索进行深度可解释的大型语言模型推理

1. 给定一个问题，ToG2.0首先执行命名实体识别（NER）和主题修剪（TP），这促使LLM评估及其出现的实体，选择一组适合作为问题起点的主题实体
	- 例如，在查询“腾讯公司的创始人中，谁曾是全国人民代表大会的成员？”时，选择一个广泛或选择不当的起点如“全国人民代表大会的成员”可能会导致陷入在大量无关数据中筛选的陷阱，从而造成耗时且不够集中的探索。有效的起点应聚焦于“腾讯公司的创始人”这样的实体。这一原则在推理任务中至关重要，尤其是在开放领域问答中，问题的高度多样性使得选择合适的起点尤为重要。
	- prompt如下：![[TOG 7.png]]
2. 基于实体“全国人民代表大会”，LLM可能会生成一个线索查询（clue query），建议收集有关他们的政治身份或隶属关系的信息。通过这种方式，ToG2.0能够更有效地引导LLM在KG中进行有针对性的探索，从而提高推理的准确性和效率。
3. 整个流程![[TOG 8.png]]
4. 流程图![[TOG 9.png]]